...
## Update (2026-09-23): manual theme switch added

The original decision above deliberately excluded a manual theme toggle in favor of OS-only `prefers-color-scheme`. The product owner asked for one directly, so it was added: a `system` / `light` / `dark` choice in Settings, backed by a small store (`src/lib/state/theme.ts`) that sets `data-theme` on `<html>` and persists the choice in `localStorage`. `src/app.css` now has an explicit `:root[data-theme="dark"]` block alongside the `prefers-color-scheme` media query, so "system" still tracks the OS live while an explicit choice overrides it. This didn't require `mode-watcher` or any other new dependency — the same token-swapping mechanism from the original decision just gained a second trigger.

Also worth recording: the first shipped version of the custom title bar didn't actually work — dragging, minimize, maximize, and close all silently failed. The cause was Tauri v2's permission system: custom app commands are unrestricted, but the _window_ commands the title bar calls (`start_dragging`, `minimize`, `maximize`, `close`, etc.) are gated by capabilities like any other core API, and `src-tauri/capabilities/default.json` only listed `core:default` plus the dialog/opener permissions already in use. `core:default` does not include these window-control permissions since they're considered sensitive enough to require explicit opt-in even for a first-party custom title bar. Fixed by adding the specific `core:window:allow-*` permissions needed.
