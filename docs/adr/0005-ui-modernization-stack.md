...
Also worth recording: the first shipped version of the custom title bar didn't actually work — dragging, minimize, maximize, and close all silently failed. The cause was Tauri v2's permission system: custom app commands are unrestricted, but the *window* commands the title bar calls (`start_dragging`, `minimize`, `maximize`, `close`, etc.) are gated by capabilities like any other core API, and `src-tauri/capabilities/default.json` only listed `core:default` plus the dialog/opener permissions already in use. `core:default` does not include these window-control permissions since they're considered sensitive enough to require explicit opt-in even for a first-party custom title bar. Fixed by adding the specific `core:window:allow-*` permissions needed.
