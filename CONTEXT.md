...
- **Plan** — the _proposed_ set of `Operation`s for a mode, computed from a scan. A plan is pure data: generating one never touches the filesystem. This is what the dry-run preview renders. The backend keeps the latest plan (`PlanStore`) and Apply refers to it by id; the webview only picks which of its operations run ([ADR 0016](docs/adr/0016-backend-owned-plans.md)).
...
- **Duplicate set** / **keeper** — in a Dedup plan, a group of files with identical contents. The **keeper** is the one file that stays put. Every other copy is a `MoveToTrash` operation tagged with the set's id (`Plan.duplicate_sets`, `Operation.duplicate_set`). The keep strategy picks the first keeper, and the user can swap it in the preview ("Keep this one"), which edits the backend's stored plan ([ADR 0021](docs/adr/0021-editing-the-stored-plan.md)).
- **Run** — the record of actually _applying_ a plan (or a subset of its selected operations). A `RunRecord` captures which operations succeeded (`AppliedOperation`, with enough information to reverse it) and which failed (`FailedOperation`, with a plain-English `error`). Runs are persisted as JSON under the OS app-data directory so History and Undo survive restarts. **History retention** removes a run once it is both outside the newest 200 and older than the `history.keep_days` setting (default 90, 0 = forever); a removed run can no longer be undone ([ADR 0020](docs/adr/0020-history-retention.md)).
